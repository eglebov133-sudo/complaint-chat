...
class Orchestrator:
    ...
    # UTM keyword → personalized welcome line
    UTM_WELCOME = {
        'банк': 'Проблемы с банком?',
        'мфо': 'Проблемы с МФО?',
        'микрозайм': 'Проблемы с микрозаймом?',
        'страхов': 'Проблемы со страховой?',
        'коллектор': 'Достали коллекторы?',
        'работодат': 'Проблемы с работодателем?',
        'зарплат': 'Не выплатили зарплату?',
        'уволь': 'Незаконное увольнение?',
        'трудов': 'Проблемы на работе?',
        'магазин': 'Проблемы с магазином?',
        'маркетплейс': 'Проблемы с маркетплейсом?',
        'wildberries': 'Проблемы с Wildberries?',
        'вайлдберриз': 'Проблемы с Wildberries?',
        'ozon': 'Проблемы с Ozon?',
        'озон': 'Проблемы с Ozon?',
        'продавц': 'Обманул продавец?',
        'возврат': 'Не возвращают деньги?',
        'управляющ': 'Проблемы с управляющей компанией?',
        'жкх': 'Проблемы с ЖКХ?',
        'тсж': 'Проблемы с ТСЖ?',
        'жэк': 'Проблемы с ЖЭК?',
        'отоплен': 'Нет отопления?',
        'протечк': 'Протечка крыши?',
        'сосед': 'Проблемы с соседями?',
        'полиц': 'Проблемы с полицией?',
        'пристав': 'Проблемы с приставами?',
        'фссп': 'Проблемы с ФССП?',
        'школ': 'Проблемы со школой?',
        'больниц': 'Проблемы с больницей?',
        'врач': 'Проблемы с врачом?',
        'поликлиник': 'Проблемы с поликлиникой?',
        'такси': 'Проблемы с такси?',
        'почт': 'Проблемы с Почтой России?',
        'застройщ': 'Проблемы с застройщиком?',
        'провайдер': 'Проблемы с провайдером?',
        'оператор': 'Проблемы с оператором связи?',
        'ростелеком': 'Проблемы с Ростелекомом?',
        'роспотреб': 'Нарушены права потребителя?',
        'прокуратур': 'Нужна помощь прокуратуры?',
        'жилищн': 'Проблемы с жильём?',
        'чиновник': 'Произвол чиновника?',
        'дорог': 'Проблемы с дорогами?',
        'автосервис': 'Проблемы с автосервисом?',
        'ресторан': 'Проблемы с рестораном?',
        'кафе': 'Проблемы с кафе?',
        'авиакомпан': 'Проблемы с авиакомпанией?',
        'рейс': 'Задержали или отменили рейс?',
        'турфирм': 'Проблемы с турфирмой?',
        'туроператор': 'Проблемы с туроператором?',
        'детский сад': 'Проблемы с детским садом?',
    }

    def _get_utm_welcome(self, state: Dict) -> str:
        """Build personalized welcome header from UTM term"""
        utm_term = state.get("data", {}).get("utm_term", "").lower()
        if not utm_term:
            return ""
        for key, title in self.UTM_WELCOME.items():
            if key in utm_term:
                return f"**{title}**\nВаша жалоба в нужных инстанциях — уже через 2 минуты.\n\n"
        return ""
